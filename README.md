...
| `ChildProfileScreen` | `src/features/child-profile/ChildProfileScreen.tsx` | Child profile dashboard (personal info, reinforcements, milestones) |
...
| `LearningThemeScreen` | `src/features/learning-theme/LearningThemeScreen.tsx` | Alternative learning theme UI entry |
...
| `changeChildrenInfo(childData)` | `src/Createchildren/index.tsx`, `src/features/child-profile/Preferences.tsx` | Create/update child profile and reinforcement data | `childData` (name/age/gender/courseDuration/reinforcements/module levels/image/style, etc.) | `{ success, message, data: children[] }` |
...
| `generateImage(description)` | `src/features/child-profile/Preferences.tsx`, `src/Procedure/ThemeSceneStep.tsx`, `src/Procedure/PronunciationStep.tsx`, `src/GptLearning/index.tsx`, `src/DisplayStoreData/index.tsx`, `src/components/GptTest/index.tsx`, `src/components/ImageGenerator/index.tsx` | Generate teaching images from prompts | image description payload (migration target: encrypted prompt payload) | image URL string |
...
- `GRPC_BASE_URL` (production should use `https://`)
- `ALLOW_INSECURE_GRPC` (optional, only for local `http://` debugging)
- `API_AES_KEY_B64` (private env only, never `EXPO_PUBLIC_*`)
- `PROMPT_AES_KEY_B64` (private env only, never `EXPO_PUBLIC_*`)
...
| `ChildProfileScreen` | `src/features/child-profile/ChildProfileScreen.tsx` | 儿童画像总览（档案、强化物、里程碑） |
...
| `LearningThemeScreen` | `src/features/learning-theme/LearningThemeScreen.tsx` | 备选学习主题页面入口 |
...
| `changeChildrenInfo(childData)` | `src/Createchildren/index.tsx`, `src/features/child-profile/Preferences.tsx` | 创建/更新儿童档案和强化物数据 | `childData`（姓名/年龄/性别/课程周期/强化物/模块等级/图片/风格等） | `{ success, message, data: children[] }` |
...
- 加密 RPC 运行时环境变量：
- `GRPC_BASE_URL`（生产环境建议 `https://`）
- `ALLOW_INSECURE_GRPC`（可选，仅用于本地 `http://` 调试）
- `API_AES_KEY_B64`（仅私有环境变量，禁止 `EXPO_PUBLIC_*`）
- `PROMPT_AES_KEY_B64`（仅私有环境变量，禁止 `EXPO_PUBLIC_*`）
