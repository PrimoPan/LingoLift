...
| `CreateChildren` | `src/Createchildren/index.jsx` | Create or update child profile data |
| `ChildProfileScreen` | `components/childProfile/ChildProfileScreen.jsx` | Child profile dashboard (personal info, reinforcements, milestones) |
...
| `HorizontalLayout` | `src/Procedure/index.jsx` | Main lesson authoring flow (theme -> scene -> modules) |
| `Draft` | `src/Draft/index.jsx` | Draft authoring/review and lesson record submission |
...
| `LearningThemeScreen` | `components/LearningTheme/LearningThemeScreen.jsx` | Alternative learning theme UI entry |
...
- `Procedure` (`ThemeSelection` -> `ThemeScene` -> `PronunciationModule` -> `Naming` -> `Language` -> `DH`): end-to-end lesson goal assembly.
...
- Business data RPC wrapper: `src/services/api.js`
- AI RPC wrapper: `src/utils/api.js`

| Function | Called by component(s) | Purpose | Main payload fields | Expected response shape |
| --- | --- | --- | --- | --- |
| `loginTeacher(username, password)` | `src/Login/Login.jsx` | Authenticate teacher and obtain session token | `username`, `password` | `{ success, token }` |
| `changeChildrenInfo(childData)` | `src/Createchildren/index.jsx`, `components/childProfile/Preferences.jsx` | Create/update child profile and reinforcement data | `childData` (name/age/gender/courseDuration/reinforcements/module levels/image/style, etc.) | `{ success, message, data: children[] }` |
| `createLearning(Goals, childName)` | `src/Draft/index.jsx` | Save finalized lesson content into child learning history | `Goals`, `childName` | `{ success, message, data: learningHistory[] }` |
| `getChildrenList()` | `src/ChildrenList/index.jsx` | Fetch all children under current teacher account | Authorization/session context | `{ success, message, data: children[] }` |
| `getChildDetails(childName)` | `src/ChildrenList/index.jsx` | Fetch selected child profile details | `name` (or child identifier) | `{ success, message, data: child }` |
| `getLearningHistoryForChild(childName)` | `src/ChildHistory/index.jsx` | Fetch sorted learning history for selected child | `name` (or child identifier) | `{ success, message, data: sortedHistory[] }` |
| `gptQuery(rawQuestion)` | `src/Procedure/ThemeSelection.jsx`, `src/Procedure/ThemeScene.jsx`, `src/Procedure/PronunciationModule.jsx`, `src/Draft/Naming.jsx`, `src/Draft/Ls.jsx`, `src/Draft/Dia.jsx`, `src/Draft/Pronun.jsx`, `src/GptLearning/index.jsx`, `src/DisplayStoreData/index.jsx`, `src/components/GptTest/index.jsx` | Text-generation requests for planning/drafting/scene suggestions | Prompt payload (migration target: encrypted prompt payload) | text or structured JSON string |
| `generateImage(description)` | `components/childProfile/Preferences.jsx`, `src/Procedure/ThemeScene.jsx`, `src/Procedure/PronunciationModule.jsx`, `src/GptLearning/index.jsx`, `src/DisplayStoreData/index.jsx`, `src/components/GptTest/index.jsx`, `src/components/ImageGenerator/index.jsx` | Generate teaching images from prompts | image description payload (migration target: encrypted prompt payload) | image URL string |
...
| `CreateChildren` | `src/Createchildren/index.jsx` | 创建或更新儿童档案 |
| `ChildProfileScreen` | `components/childProfile/ChildProfileScreen.jsx` | 儿童画像总览（档案、强化物、里程碑） |
...
| `HorizontalLayout` | `src/Procedure/index.jsx` | 主备课流程（主题 -> 场景 -> 模块） |
| `Draft` | `src/Draft/index.jsx` | 教案草稿编辑/查看与学习记录提交 |
| `GptLearning` | `src/GptLearning` | AI 场景建议与背景图生成 |
| `DisplayStoreData` | `src/DisplayStoreData` | 生成教学步骤与素材图 |
| `LearningThemeScreen` | `components/LearningTheme/LearningThemeScreen.jsx` | 备选学习主题页面入口 |
...
- `Procedure`（`ThemeSelection` -> `ThemeScene` -> `PronunciationModule` -> `Naming` -> `Language` -> `DH`）：完成备课目标构建。
...
- 业务数据接口：`src/services/api.js`
- AI 接口：`src/utils/api.js`

| 函数 | 调用组件 | 用途 | 主要请求字段 | 期望返回结构 |
| --- | --- | --- | --- | --- |
| `loginTeacher(username, password)` | `src/Login/Login.jsx` | 教师登录并获取会话令牌 | `username`, `password` | `{ success, token }` |
| `changeChildrenInfo(childData)` | `src/Createchildren/index.jsx`, `components/childProfile/Preferences.jsx` | 创建/更新儿童档案和强化物数据 | `childData`（姓名/年龄/性别/课程周期/强化物/模块等级/图片/风格等） | `{ success, message, data: children[] }` |
| `createLearning(Goals, childName)` | `src/Draft/index.jsx` | 保存最终教案到儿童学习历史 | `Goals`, `childName` | `{ success, message, data: learningHistory[] }` |
| `getChildrenList()` | `src/ChildrenList/index.jsx` | 查询当前教师名下儿童列表 | 授权上下文 | `{ success, message, data: children[] }` |
| `getChildDetails(childName)` | `src/ChildrenList/index.jsx` | 查询指定儿童详细档案 | `name`（或儿童标识） | `{ success, message, data: child }` |
| `getLearningHistoryForChild(childName)` | `src/ChildHistory/index.jsx` | 查询并返回时间倒序历史记录 | `name`（或儿童标识） | `{ success, message, data: sortedHistory[] }` |
| `gptQuery(rawQuestion)` | `src/Procedure/ThemeSelection.jsx`, `src/Procedure/ThemeScene.jsx`, `src/Procedure/PronunciationModule.jsx`, `src/Draft/Naming.jsx`, `src/Draft/Ls.jsx`, `src/Draft/Dia.jsx`, `src/Draft/Pronun.jsx`, `src/GptLearning/index.jsx`, `src/DisplayStoreData/index.jsx`, `src/components/GptTest/index.jsx` | 文本生成（场景、目标、草稿、步骤） | prompt 内容（迁移目标：加密 prompt 载荷） | 文本或 JSON 字符串 |
| `generateImage(description)` | `components/childProfile/Preferences.jsx`, `src/Procedure/ThemeScene.jsx`, `src/Procedure/PronunciationModule.jsx`, `src/GptLearning/index.jsx`, `src/DisplayStoreData/index.jsx`, `src/components/GptTest/index.jsx`, `src/components/ImageGenerator/index.jsx` | 根据描述生成教学图片 | 图片描述（迁移目标：加密 prompt 载荷） | 图片 URL 字符串 |
