...
import ProcedureStepTabs from './ProcedureStepTabs';
import ThemeSelectionStep from './ThemeSelectionStep';
import ProcedureNavigationButtons from './ProcedureNavigationButtons';
import ThemeSceneStep from './ThemeSceneStep';
import PronunciationStep from './PronunciationStep';
import NamingGoalPlanner from './NamingGoalPlanner';
import LanguageGoalPlanner from './LanguageGoalPlanner';
import DialogueGoalPlanner from './DialogueGoalPlanner';
...
const Procedure = () => {
    ...
    const [selectedTheme, setSelectedTheme] = useState(STEP_LABELS[0]);
    ...
    const safeAvailableModulesIndex = availableModules
        .map((mod) => STEP_LABELS.indexOf(mod))
        .filter((idx) => idx >= 0);
    ...
    // 6. 同步 selectedTheme
    useEffect(() => {
        setSelectedTheme(STEP_LABELS[currentStep]);
    }, [currentStep]);
    ...
    return (
        <View style={[styles.container, { width, height }]}>
            ...
            {currentStep === 3 && <NamingGoalPlanner onSelectGoal={setNamingGoal} />}
            {currentStep === 4 && <LanguageGoalPlanner onSelectGoal={setLgGoal} />}
            {currentStep === 5 && <DialogueGoalPlanner onSelectGoal={setDhGoal} />}

            {/* 按钮组 */}
            <ProcedureNavigationButtons handleNext={handleNextStep} handleLast={handleLast} step={currentStep} />
        </View>
    );
};
